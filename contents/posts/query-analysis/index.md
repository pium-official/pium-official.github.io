...
> 이 글은 우테코 피움팀 크루 '[그레이](https://github.com/kim0914)', '[하마드](https://github.com/rawfishthelgh)'가 작성했습니다.
...
## 불필요한 디스크 I/O 발생을 커버링 인덱스로 처리하기

![](.index_images/img_12.png)

위는 히스토리의 타입을 조회하는 쿼리입니다.
비교 조건인 "history_type" 칼럼에는 유니크 제약 조건이 있기에, 
위 쿼리는 세컨더리 인덱스를 이용해 데이터를 가져옵니다.

그러나, select를 통해 가져오려는 칼럼에, 인덱스가 존재하지 않는 "create_at", "updated_at"이 존재하고 있어,
 이는 결국 디스크 I/O를 유발하게 됩니다. 따라서 실행계획의 Extra 칼럼에 "null"로 되죠.

따라서 두 가지 조건을 선택할 수 있습니다.
1. select 조건에 id, history_type 만을 남겨둔다.
2. 테이블에서 "create_at", "updated_at" 칼럼을 삭제한다.

저희는 2번을 선택했습니다. "create_at", "updated_at" 칼럼이 존재하는 이유는, 
어떤 사용자가 데이터를 언제 저장했는지 시각을 알기 위한 장치였습니다.
하지만 히스토리 타입은 단순히 이름일 뿐, 저장 시각이 불필요하다고 생각했습니다.
또한 2번을 선택하면 알아서 1번의 조건이 충족되기도 합니다.

![](.index_images/img_13.png)

따라서, 해당 컬럼을 제거하고 다시 실행계획을 확인하면, 
다음과 같이 실행계획의 "Extra" 칼럼에서 "Using Index"로 커버링 인덱스 처리가 완료됨을 확인할 수 있습니다.
디스크 I/O를 차단하는 효과가 있었습니다.
